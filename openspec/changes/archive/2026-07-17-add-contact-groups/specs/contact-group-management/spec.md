...
#### Scenario: Empty criterion does not restrict membership
- **WHEN** a group has no selected values for one supported criterion
- **THEN** that criterion does not exclude otherwise matching contacts
...
#### Scenario: Include non-matching contact
- **WHEN** a user manually includes a contact that does not match the group criteria
- **THEN** the contact is included in the group
...
The system SHALL allow a contact to belong to zero, one, or multiple groups.

#### Scenario: Contact belongs to several groups
- **WHEN** a contact matches or is included in more than one group
- **THEN** the system preserves each group membership independently
